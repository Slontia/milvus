...
func validateRLSPolicyExpressionsWithSchema(schema *schemapb.CollectionSchema, usingExpr string, checkExpr string) error {
	...
	if err := validateRLSPolicyExpression(schemaHelper, "using", usingExpr); err != nil {
		return err
	}
	return validateRLSPolicyExpression(schemaHelper, "check", checkExpr)
}

func validateRLSPoliciesWithSchema(ctx context.Context, meta IMetaTable, dbName string, collectionName string, schema *schemapb.CollectionSchema) error {
	policies, err := meta.ListRLSPolicies(ctx, &rlsutil.ListRowPoliciesRequest{
		DbName:         dbName,
		CollectionName: collectionName,
	})
	if err != nil {
		return merr.Wrap(err, "failed to list RLS policies for schema dependency check")
	}
	for _, policy := range policies {
		if policy == nil {
			continue
		}
		if err := validateRLSPolicyExpressionsWithSchema(schema, policy.GetUsingExpr(), policy.GetCheckExpr()); err != nil {
			return merr.Wrapf(err, "RLS policy %q is incompatible with schema change", policy.GetPolicyName())
		}
	}
	return nil
}

func collectRLSPolicyFieldRefs(ctx context.Context, meta IMetaTable, dbName string, collectionName string, coll *model.Collection) (map[int64][]string, error) {
	policies, err := meta.ListRLSPolicies(ctx, &rlsutil.ListRowPoliciesRequest{
		DbName:         dbName,
		CollectionName: collectionName,
	})
	if err != nil {
		return nil, merr.Wrap(err, "failed to list RLS policies for dependency check")
	}
	...
	for _, policy := range policies {
		...
		for _, expr := range []string{policy.GetUsingExpr(), policy.GetCheckExpr()} {
			refs, err := collectRLSPolicyExprFieldRefs(schemaHelper, expr)
			if err != nil {
				return nil, merr.Wrapf(err, "failed to inspect RLS policy %q dependencies", policy.GetPolicyName())
			}
			for fieldID := range refs {
				fieldRefs[fieldID] = append(fieldRefs[fieldID], policy.GetPolicyName())
			}
		}
	}
...
func validateRLSNoReferencedFieldDropped(ctx context.Context, meta IMetaTable, dbName string, collectionName string, coll *model.Collection, droppedFieldIDs []int64) error {
	if len(droppedFieldIDs) == 0 {
		return nil
	}
	fieldRefs, err := collectRLSPolicyFieldRefs(ctx, meta, dbName, collectionName, coll)
	if err != nil {
		return err
	}
...
func validateRLSFunctionOutputNotReferenced(ctx context.Context, meta IMetaTable, dbName string, collectionName string, coll *model.Collection, fn *model.Function, operation string) error {
...
func validateRLSPolicyExpression(schemaHelper *typeutil.SchemaHelper, exprKind string, expr string) error {
	expr = strings.TrimSpace(expr)
	if expr == "" {
		return nil
	}
...
func validateRLSParsedExpr(expr *planpb.Expr, allowedTemplateVariables map[string]struct{}) error {
	...
	case *planpb.Expr_UnaryExpr:
		...
		return validateRLSParsedExpr(node.UnaryExpr.GetChild(), allowedTemplateVariables)
	case *planpb.Expr_UnaryRangeExpr:
	...
	case *planpb.Expr_BinaryExpr:
		...
		case planpb.BinaryExpr_LogicalAnd, planpb.BinaryExpr_LogicalOr:
			if err := validateRLSParsedExpr(node.BinaryExpr.GetLeft(), allowedTemplateVariables); err != nil {
				return err
			}
			return validateRLSParsedExpr(node.BinaryExpr.GetRight(), allowedTemplateVariables)
		default:
			return merr.WrapErrParameterInvalidMsg("unsupported RLS binary operator %s", node.BinaryExpr.GetOp().String())
		}
	default:
...
func validateRLSTermExpr(expr *planpb.TermExpr, allowedTemplateVariables map[string]struct{}) error {
	...
	if len(expr.GetValues()) > maxElements {
...
func validateRLSJSONContainsExpr(expr *planpb.JSONContainsExpr, allowedTemplateVariables map[string]struct{}) error {
	...
	maxElements := Params.ProxyCfg.RLSMaxArrayLiteralElements.GetAsInt()
	if len(expr.GetElements()) > maxElements {
		return merr.WrapErrParameterInvalidMsg("RLS policy expression exceeds max array literal elements %d", maxElements)
	}
...
func (mt *MetaTable) PrepareCreateRLSPolicy(ctx context.Context, req *rlsutil.CreateRowPolicyRequest, policyID int64) (*model.RLSPolicy, error) {
	...
	existingPolicy, err := mt.catalog.GetRLSPolicy(ctx, coll.CollectionID, req.GetPolicyName())
	if err == nil {
		if rlsPolicyMatchesCreateRequest(existingPolicy, req) {
			policy := model.CloneRLSPolicy(existingPolicy)
			policy.DBID = coll.DBID
			policy.CollectionID = coll.CollectionID
			return policy, nil
		}
		return nil, merr.WrapErrParameterInvalidMsg("RLS policy [%s] already exists with a different definition", req.GetPolicyName())
	} else if !errors.Is(err, merr.ErrIoKeyNotFound) {
		return nil, merr.Wrap(err, "failed to get RLS policy")
	}

	policies, err := mt.catalog.ListRLSPolicies(ctx, coll.CollectionID)
	if err != nil {
...
func rlsPolicyMatchesCreateRequest(policy *model.RLSPolicy, req *rlsutil.CreateRowPolicyRequest) bool {
	return policy != nil && req != nil &&
		policy.PolicyName == req.GetPolicyName() &&
		policy.PolicyType == req.GetPolicyType() &&
		slices.Equal(policy.Actions, req.GetActions()) &&
		policy.UsingExpr == req.GetUsingExpr() &&
		policy.CheckExpr == req.GetCheckExpr() &&
		policy.Description == req.GetDescription()
}
...
func (mt *MetaTable) PrepareSetRLSPrincipalTags(ctx context.Context, req *rlsutil.SetRLSPrincipalTagsRequest) (*model.RLSPrincipal, error) {
	...
	principalExists := lo.ContainsBy(principals, func(principal *model.RLSPrincipal) bool {
		return principal != nil && principal.PrincipalName == req.GetPrincipalName()
	})
	if !principalExists && len(principals) >= Params.ProxyCfg.RLSMaxPrincipalsPerCollection.GetAsInt() {
