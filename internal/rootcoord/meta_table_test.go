...
func TestMetaTable_RLSMetadata(t *testing.T) {
	...
	t.Run("create duplicate and list policies", func(t *testing.T) {
		...
		catalog.EXPECT().SaveRLSPolicy(mock.Anything, mock.MatchedBy(func(policy *model.RLSPolicy) bool {
			return policy.PolicyID == 100 && policy.PolicyName == "dept_read"
		})).Return(nil).Once()
		collectionID, err = meta.CreateRLSPolicy(ctx, createReq, 101)
		require.NoError(t, err)
		require.Equal(t, int64(20), collectionID)
		require.Len(t, meta.collID2Meta[20].RLSPolicies, 1)
		...
		require.Contains(t, err.Error(), "different definition")
	...
	t.Run("validate policy expressions", func(t *testing.T) {
		...
		quotedExpr := `dept == "$current_principal" and owner == "$current_principal_tags['owner']"`
	...
	t.Run("rls limits use paramtable", func(t *testing.T) {
		...
		catalog.EXPECT().ListRLSPolicies(mock.Anything, int64(20)).Return([]*model.RLSPolicy{{PolicyName: "existing"}}, nil).Once()
		_, err = meta.CreateRLSPolicy(ctx, &rlsutil.CreateRowPolicyRequest{
			DbName:         "db1",
			CollectionName: "coll1",
			PolicyName:     "p",
	...
	t.Run("reject combined expression length on create and update", func(t *testing.T) {
		...
		catalog.EXPECT().ListRLSPolicies(mock.Anything, int64(20)).Return([]*model.RLSPolicy{otherPolicy, oldPolicy}, nil).Once()
		_, err = meta.UpdateRLSPolicy(ctx, &rlsutil.UpdateRowPolicyRequest{
			DbName:         "db1",
			CollectionName: "coll1",
			PolicyName:     "existing",
	...
	t.Run("principal tags", func(t *testing.T) {
		...
		catalog.EXPECT().ListRLSPrincipals(mock.Anything, int64(20)).Return([]*model.RLSPrincipal{{PrincipalName: "alice"}}, nil).Once()
		...
		principals, err := meta.ListRLSPrincipals(ctx, &rlsutil.ListRLSPrincipalsRequest{DbName: "db1", CollectionName: "coll1"})
		require.NoError(t, err)
