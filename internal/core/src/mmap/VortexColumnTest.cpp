...
constexpr int64_t kScanBatchRows = 3;
...
CollectIntScanValues(VortexColumn& column, int64_t start, int64_t length) {
    ...
    ChunkedColumnInterface::ScanBatch batch;
    while (result->Next(kScanBatchRows, &batch)) {
...
CheckNullableFilteredScanReturnsValidity(VortexColumn& column, DataType type) {
    ...
    while (scan_result->Next(kScanBatchRows, &scan_batch)) {
...
TEST(VortexColumnTest, MultiFileTakeAndScan) {
    ...
    ChunkedColumnInterface::ScanBatch batch;
    while (scan_result->Next(kScanBatchRows, &batch)) {
...
TEST(VortexColumnTest, PreservesMaterializedNormalizationForDataScan) {
    ...
    ASSERT_TRUE(timestamp_scan->Next(2, &timestamp_batch));
    ...
    EXPECT_FALSE(timestamp_scan->Next(2, &timestamp_batch));
